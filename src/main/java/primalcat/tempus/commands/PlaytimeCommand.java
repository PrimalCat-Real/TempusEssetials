...
//        getLogger().info("run playtime " + Integer.parseInt(PlaceholderAPI.setPlaceholders(player, "%plan_player_time_active_raw%")) / 3600);
//
//        getLogger().info("run playtime s" + Integer.parseInt(PlaceholderAPI.setPlaceholders(player, "%plan_player_time_active_raw%")));
//        getLogger().info("run playtime old" + player.getStatistic(Statistic.PLAY_ONE_MINUTE) / 20);
        int totalPlaytimeInSeconds;
        try{
            totalPlaytimeInSeconds = Math.round((float) Integer.parseInt(PlaceholderAPI.setPlaceholders(player, "%plan_player_time_active_raw%")) / 1000);
        }catch (Exception e){
            totalPlaytimeInSeconds = player.getStatistic(Statistic.PLAY_ONE_MINUTE) / 20;
        }
...
//        int totalPlaytimeInSeconds = player.getStatistic(Statistic.PLAY_ONE_MINUTE) / 20;
        ...
        formattedPlaytimeFuture.thenAccept(formattedPlaytime -> {
            ...
            sender.sendMessage("§7Проведено в игре: §6" + formattedPlaytime);
        });

        Integer hoursPlayedFuture = Util.getHoursPlayedAsync(player);
        Integer minutesPlayedFuture = Util.getMinutesPlayedAsync(player);
        Integer secondsPlayedFuture = Util.getSecondsPlayedAsync(player);


        int hoursPlayed = hoursPlayedFuture; // .join() to get the result
        int minutesPlayed = minutesPlayedFuture;
        int secondsPlayed = secondsPlayedFuture;
